...
def csvPrinter(parsedDict: dict, depth = 0):
    output = ""
    for i, key in enumerate(parsedDict.keys()):
        element = parsedDict[key]
        last_str = ""
        if type(element) is not dict:
            last_str = ";" * depth + f'{key}: {element}'
        else:
            output += "\n" +";" * depth + f'{key}: '
            output += csvPrinter(element, depth + 1)
            last_str = ";" * depth
...
k = csvPrinter(parsedDict)
open("schedule.csv", 'w', encoding='utf-8').write(k)